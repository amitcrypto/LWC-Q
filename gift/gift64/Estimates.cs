...
{ 
    class Estimates  
    {
        ...
        {
            var sim = getTraceSimulator(false);
            var res = QTests.GIFT.SubCells.Run(sim, nQBits(64, false), free_swaps).Result;
            ProcessSim<Qop>(sim, comment);
            sim = getTraceSimulator(true);
            res = QTests.GIFT.SubCells.Run(sim, nQBits(64, false), free_swaps).Result;
            ProcessSim<Qop>(sim, comment, true); 
        }
        ...
        {
            var sim = getTraceSimulator(false);
            var res = QTests.GIFT.PermBits.Run(sim, nQBits(64, false), free_swaps).Result;
            ProcessSim<Qop>(sim, comment);
            sim = getTraceSimulator(true);
            res = QTests.GIFT.PermBits.Run(sim, nQBits(64, false), free_swaps).Result;
            ProcessSim<Qop>(sim, comment, true);
        }
        ...
        public static void KeySchedule<Qop>(string comment = "", int round = 28, bool free_swaps = true)
        {
            var sim = getTraceSimulator(false);
        ...
        public static void AddConstants<Qop>(string comment = "", int round = 28, bool free_swaps = true)
        {
            var sim = getTraceSimulator(false);
            var res = QTests.GIFT.AddConstants.Run(sim, nQBits(64, false), round, true).Result;
            ProcessSim<Qop>(sim, comment);
            sim = getTraceSimulator(true);
            res = QTests.GIFT.AddConstants.Run(sim, nQBits(64, false), round, true).Result;
            ProcessSim<Qop>(sim, comment, true);
        }
        ...
        {
            var sim = getTraceSimulator(false);
            var res = QTests.GIFT.Round.Run(sim, nQBits(64, false), nQBits(128, false), round, free_swaps).Result;
            ProcessSim<Qop>(sim, comment);
            sim = getTraceSimulator(true);
            res = QTests.GIFT.Round.Run(sim, nQBits(64, false), nQBits(128, false), round, free_swaps).Result;
            ProcessSim<Qop>(sim, comment, true); 
        }

        public static void Gift<Qop>(string comment = "", int round = 28, bool free_swaps = true, string suffix = "")
        {
            var sim = getTraceSimulator(false);
            var res = QTests.GIFT.Gift.Run(sim, nQBits(64, false), nQBits(128, false), round, free_swaps).Result;
            ProcessSim<Qop>(sim, comment, false, suffix);
            sim = getTraceSimulator(true);
            res = QTests.GIFT.Gift.Run(sim, nQBits(64, false), nQBits(128, false), round, free_swaps).Result;
            ProcessSim<Qop>(sim, comment, true);
        }

        public static void GroverOracle<Qop>(string comment = "", int pairs = 1, int round = 28, bool free_swaps = true, string suffix = "")
        {
            var sim = getTraceSimulator(false);
            var res = QTests.GIFT.GroverOracle.Run(sim, nQBits(128, false), nQBits(64*pairs, false), nBits(64*pairs, false), pairs, round, free_swaps).Result;
            ProcessSim<Qop>(sim, comment, false, suffix);
            sim = getTraceSimulator(true);
            res = QTests.GIFT.GroverOracle.Run(sim, nQBits(128, false), nQBits(64*pairs, false), nBits(64*pairs, false), pairs, round, free_swaps).Result;
            ProcessSim<Qop>(sim, comment, true); 
        }
    }
}
